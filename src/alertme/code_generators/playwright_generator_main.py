...
from alertme.web_voyager.tasks.tennis_task import (
    get_prompt,
    get_ai_generated_start_code,
    get_task_params,
    # get_html_parser,
    custom_code_task_guideance,
)
...
if __name__ == "__main__":
    ...
    codegen_prompt = get_generalize_code_prompt(
        test_cases, get_ai_generated_start_code(), get_prompt(), playwright_actions_code_lib
    )
    extra_task_guideance = custom_code_task_guideance()
    codegen_prompt += extra_task_guideance
    code_generator = CodeGenerator(
        codegen_prompt,
        start_at_condense=False,
        use_cached_navigation_code=True,
