...
from alertme.web_voyager.playwright_actions_core import (
    click as playwright_click,
    type_text as playwright_type_text,
    scroll_until_visible as playwright_scroll,
    go_back as playwright_go_back,
    to_google as playwright_to_google,
    to_url as playwright_to_url,
)
from alertme.web_voyager.prompt import (
...
from alertme.web_voyager.tasks import tennis_task
...
class TaskParamCode(BaseModel):
    code_thought: str = Field(
        description="Thought: Your brief thoughts on how you plan to extract input_string to output_string."
    )
...
class TaskParamSelection(BaseModel):
    ...
    task_param_used: str = Field(
        description="Name of the task_param used to fill the [Text] in action string."
    )
...
class Reflection(BaseModel):
    ...
    intention_of_action: str = Field(description="Intention of the attempted action.")
    ...
    action_summary: Optional[str] = Field(
        description="In a few words describe the action taken and the result."
    )
    ...
    try:
        ...
        store_action(
            "click",
            {
                ...
                "text": bbox.get("text", ""),
                "parent_html": bbox.get("parentHTML", ""),
                "task_param_used": task_param_used,
            },
        )
...
{state['action_summary']}
    ...
    return {
        **state,
        "scratchpad": old,
        "formatted_scratchpad": formatted_scratchpad,
        "step": step,
    }  # "previous_action_summaries" : previous_action_summaries}
...
async def generate_task_param_code(state: AgentState):
    ...
    if task_param_used not in state["task_params"]:
        # TODO(P3): Feed this back to the agent to regenerate the action with correct args.
        print(
            f"Warning: Task param {task_param_used} not found in task params {state['task_params']}. Replacing with error_task_param_used_not_found."
        )
    ...
    print(
        f"Generating task param code for {task_param_used} with value {task_param_value} and action text {action_text}"
    )
    start_time_llm = asyncio.get_event_loop().time()
    task_param_code_obj = await chain.ainvoke(
        {"input_string": task_param_value, "output_string": action_text},
        {"tags": ["generate-task-param-code"]},
    )
...
async def reflect(state: AgentState):
    ...
    start_time = asyncio.get_event_loop().time()
    comparison_result = await compare_chain.ainvoke(
        {
        ...
        {"tags": ["reflect-on-diff", prediction.action, run_id]},
    )
    end_time = asyncio.get_event_loop().time()
...
async def call_agent(task: str, task_params: dict, max_steps: int = 300):
    ...
    await graph.ainvoke({"scratchpad": list(), "task": task, "task_params": task_params}, config)
...
if __name__ == "__main__":
    run_web_voyager(
        task=tennis_task.get_prompt(), task_params=tennis_task.get_task_params(), wait_for_tracers=True
    )
