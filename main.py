...
def print_request_response_summary(pairs, total_entries, requirement):
    ...
    for i, (request, response) in enumerate(pairs):
        ...
        if index != -1:
            ...
            end = min(len(response_text), index +
                      DEBUG_CONTEXT_SIZE + len(requirement))
...
filtered_request_response_pairs = extract_filtered_request_response_pairs(
    har_data, requirement, strategy)
total_entries = len(har_data['log']['entries']
                    ) if 'log' in har_data and 'entries' in har_data['log'] else 0

print_request_response_summary(
    filtered_request_response_pairs, total_entries, requirement)

# Writing the first response to a file
with open('data/response.txt', 'w', encoding='utf-8') as file:
    first_response = filtered_request_response_pairs[0][1]['content'].get(
        'text', '') if filtered_request_response_pairs else "No response available."
...
# TODO: Add the LLM powered way to extract the times from codegen.py.
