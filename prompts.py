def get_reflection_prompt(last_test_results, condensed_e2e_nav_code, condensed_e2e_nav_code_summary):
...
If useful, consider the Debug Output when reasoning about your response.
    ...
    If you have a theory about what went wrong, consider the HTML and xpaths in the test results to double check your theory.
    ...
    (4) Here is the code that has been tested to get us to the final page:
    ...
    You should not depart too far from this code, except the last few lines which are the final code that accomplishes the main objective to pass tests.
    If you have not arrived at a page where the final action is possible, you should check that you did not leave out any steps in the navigation code above.
    """
...
def get_code_prompt_tuple(CODE_SUFFIX):
    ...
    The "page" argument is a playwright page object.
    VERY IMPORTANT: Javascript/Playwright tips:
        - If page.click times out waiting for element to be available, it may be hidden or not visible, in that case, \
        try javascript like page.evaluate(f'document.querySelector(...).click()').
        - If visibility issues continue or javascript code continues to fail, you may try finding the LOCATION of
        an element and clicking on the COORDINATE, especially for elements that require multiple sequential click interactions.
        - Code: page.mouse.click(coords['x'], coords['y']), and you just need to use javascript to find the coordinates of the element(s) to click.
        - E.g. To operate a dropdown, first find the location of the dropdown, then click on the coordinate, then click on the dropdown element.
        - Making sure an element is visible before clicking on it by setting a visibilty variable to true usually DOES NOT WORK. Try the above instead.
    ...
    Timeouts:
        ...
        - IMPORTANT: If you time out looking for an element it is UNLIKELY that increasing the timeout will help.
        DO NOT INCREASE TIMEOUTS! Instead you should try to find a different strategy such as clicking on a coordinate
        or using javascript to click on an element.
    Exceptions
        - Don't raise uncaught exceptions. Instead, append to debug output and continue execution.
    IMPORTANT: Use Playwright library not selenium.
     - Be sure to use the playwright web driver and use the page object provided to you via the get_page() method above.     
    IMPORTANT: NEVER USE ASYNC PLAYWRIGHT, USE THE page you are given in get_page() which is spawned externally by the sync PlaywrightRunner class.
